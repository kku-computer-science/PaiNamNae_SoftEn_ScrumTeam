# -*- coding: utf-8 -*-
*** Settings ***
Documentation     UAT_TEST14 Passenger Transfer only (separated)
...
${BTN_METHOD_TRANSFER}          xpath=//button[contains(normalize-space(),'โอนเงิน')]

${QR_PROMPTPAY}                 xpath=//img[@alt='QR Code พร้อมเพย์']
${NO_PROMPTPAY_WARNING}         xpath=//*[contains(normalize-space(),'คนขับไม่ได้ตั้งค่าพร้อมเพย์')]
${BANK_SECTION_TEXT}            xpath=//*[contains(normalize-space(),'บัญชีธนาคาร') or contains(normalize-space(),'โอนผ่านบัญชีธนาคาร')]
...
*** Test Cases ***
UAT14-Passenger-Transfer-Payment-Only
    Open Website And Login
    Open My Trip Page And Select All
    Open Payment Popup
    Verify Payment Popup Step1
    Choose Pay For Friend And Transfer
    Upload Slip And Go Confirm
...
Choose Pay For Friend And Transfer
    ...
    Click Element    ${BTN_METHOD_TRANSFER}
    IF    '${friend_status}' == 'PASS'
        Wait Until Page Contains    รวมทั้งหมด    timeout=10s
    END
    Transfer Channel Should Be Ready

Transfer Channel Should Be Ready
    ${has_qr}=    Run Keyword And Return Status    Element Should Be Visible    ${QR_PROMPTPAY}
    ${has_no_pp_warn}=    Run Keyword And Return Status    Element Should Be Visible    ${NO_PROMPTPAY_WARNING}
    ${has_bank_info}=    Run Keyword And Return Status    Element Should Be Visible    ${BANK_SECTION_TEXT}
    IF    ${has_qr} or ${has_no_pp_warn} or ${has_bank_info}
        RETURN
    END
    Fail    ไม่พบช่องทางโอนเงิน (QR/ข้อความเตือนพร้อมเพย์/ข้อมูลบัญชีธนาคาร)

Upload Slip And Go Confirm
    Click Button    ${BTN_ATTACH_SLIP}
    Wait Until Page Contains    แนบหลักฐานการโอน    timeout=10s
    Wait Until Page Contains Element    ${FILE_INPUT}    timeout=10s
    ${slip_file_canonical}=    Normalize Path    ${SLIP_FILE_PATH}
    File Should Exist    ${slip_file_canonical}
...
Submit Transfer Evidence
    ...
    Run Keyword And Ignore Error    Wait Until Page Contains    ส่งหลักฐานสำเร็จ    timeout=8s
